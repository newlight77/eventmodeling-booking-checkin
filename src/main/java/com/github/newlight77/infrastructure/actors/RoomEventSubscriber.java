...
import com.github.newlight77.infrastructure.eventstore.ISubscriber;
...
public class RoomEventSubscriber implements ISubscriber<CheckedInEventCreated> {
    ...
    public void subscribe(Consumer<CheckedInEventCreated> callback) {
        final ActorSystem system = ActorSystem.create();
        ...
        final Closeable closeable = connection.subscribeToAll(new SubscriptionObserver<IndexedEvent>() {
            ...
            public void onEvent(IndexedEvent indexedEvent, Closeable subscription) {
                ...
                try {
                    ...
                    callback.accept((CheckedInEventCreated) object); // WIP
